    ...
    const filteredMarkets = currentMarkets
        .filter(market => {
            ...
            const matchesCategory =
                ...
                (activeCategory === 'News' && market.category === 'NEWS');
            if (!matchesCategory) return false;

            // Hide ACTIVE markets whose deadline has already passed
            // (bridge service will move them to RESOLVING shortly)
            if (activeMarketState === 'ACTIVE') {
                const deadlineTs = typeof market.deadline === 'string'
                    ? parseInt(market.deadline, 10) || 0
                    : market.deadline || 0;
                if (deadlineTs > 0 && deadlineTs < now) return false;
            }
        ...
        .sort((a, b) => {
            ...
            // For RESOLVED and UNDETERMINED markets, show newest (latest deadline) first
            // as they are the most recently completed ones.
            if (activeMarketState === 'RESOLVED' || activeMarketState === 'UNDETERMINED') {
                return bTime - aTime;
            }

            // For ACTIVE and RESOLVING, show soonest ending first (earliest deadline)
            return aTime - bTime;
        });
