...
import { createBet, createNewsBet, resolveBet, setCreatorApproval } from '../../lib/onchain/adminWrites';
...
const AdminPage: React.FC = () => {
    ...
    const handleCreate = async () => {
        ...
        const isNews = Number(form.resolutionType) === 2;
        const endDateMs = new Date(form.endDate).getTime();

        // Validation
        if (!form.title.trim()) { alert('Title is required.'); return; }
        if (!form.sideAName.trim()) { alert('Side A name is required.'); return; }
        if (!form.sideBName.trim()) { alert('Side B name is required.'); return; }
        if (!form.endDate || isNaN(endDateMs)) { alert('End date is required.'); return; }
        if (endDateMs <= Date.now()) { alert('End date must be in the future.'); return; }
        if (isNews && !form.question.trim()) { alert('Question is required for NEWS markets.'); return; }
        if (isNews && !form.evidenceUrl.trim()) { alert('Evidence URL is required for NEWS markets.'); return; }

        setCreating(true);
        try {
            const endDateSeconds = Math.floor(endDateMs / 1000);

            // NEWS markets use createNewsBet; others use createBet
            let txHash: `0x${string}` | undefined;
            if (isNews) {
                txHash = await createNewsBet({
                    title: form.title,
            ...
            if (txHash) {
                ...
                try {
                    const parsedLogs = parseEventLogs({
                        abi: BetFactoryArtifact.abi,
                        logs: receipt.logs,
                        eventName: 'BetCreated'
                    }) as BetCreatedEventLog[];

                    if (parsedLogs.length > 0) {
                        const betCreatedEvent = parsedLogs[0];
                        contractAddress = betCreatedEvent.args?.betAddress ?? null;
                ...
                <div className={styles.card}>
                    ...
                    <div className={styles.formGrid}>
                        ...
                        <select className={styles.input} value={form.resolutionType} onChange={(e) => setForm({ ...form, resolutionType: e.target.value })}>
                            ...
                            <option value="2">NEWS (AI Oracle)</option>
                        </select>
                        {Number(form.resolutionType) === 2 ? (
                            <>
                                <input className={styles.input} placeholder="Question (e.g., Did X happen?)" value={form.question} onChange={(e) => setForm({ ...form, question: e.target.value })} />
                                <input className={styles.input} placeholder="Evidence URL (open-access news source)" value={form.evidenceUrl} onChange={(e) => setForm({ ...form, evidenceUrl: e.target.value })} />
                            </>
                        ) : (
                            <>
