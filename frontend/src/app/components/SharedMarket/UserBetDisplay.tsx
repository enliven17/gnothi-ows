...
const UserBetDisplay: React.FC<UserBetDisplayProps> = ({ market }) => {
    ...
    const fetchUserBetData = React.useCallback(async () => {
        ...
        setLoading(true);
        try {
            const [userBets, winnings, claimState] = await Promise.all([
                ...
                getUserClaimState(market.contractId as `0x${string}`, walletAddress as `0x${string}`)
            ]);

            setBetData({
                ...userBets,
                ...winnings,
                claimableAmount: claimState.claimableAmount,
                hasClaimed: claimState.hasClaimed
            });
        } catch (error) {
        ...
        return (
            <div className={styles.container}>
                ...
                <div className={styles.noBets}>
                    You have not placed any bets on this market yet.
                </div>
            </div>
        );
    }
    ...
    const winningsAmount = isResolved ? betData.claimableAmount : (userWonA ? betData.ifSideAWins : userWonB ? betData.ifSideBWins : 0);
        ...
        <div className={styles.container}>
            <div className={styles.header}>
                ...
                {isResolved && (
                    <span className={`${styles.badge} ${userWon ? styles.badgeWon : styles.badgeLost}`}>
                        {betData.hasClaimed ? 'Claimed' : userWon ? 'Won' : 'Lost'}
                    </span>
                )}
            </div>
            ...
            {userWon && isResolved && (
                <button
                    ...
                    onClick={handleClaim}
                    disabled={claiming || betData.hasClaimed || winningsAmount <= 0}
                >
                    {claiming ? 'Claiming...' : betData.hasClaimed
                        ? 'Already claimed'
