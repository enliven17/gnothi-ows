...
import { getGenLayerTxHash } from '../api/resolutionApi';
...
async function fetchMarketInfo(betAddress: `0x${string}`): Promise<MarketData> {
    ...
    const isNews = resolutionTypeNum === 2;
    const category = isNews ? "NEWS" : (resolutionTypeNum === 1 ? "STOCKS" : "CRYPTO");
    const mType = isNews ? "other" : category === "STOCKS" ? "stock" : "crypto";
    // For NEWS: decodedSymbol = question, decodedName = evidenceUrl
    const identifier = isNews
        ? decodedName     // evidenceUrl used as identifier
        : category === "STOCKS"
            ? decodedSymbol
            : (decodedName || decodedSymbol);

    // Fetch GenLayer oracle tx hash for NEWS markets in non-ACTIVE states
    let genLayerTxHash: string | undefined;
    if (resolutionTypeNum === 2 && status !== 'ACTIVE') {
        genLayerTxHash = (await getGenLayerTxHash(betAddress)) ?? undefined;
    }
    ...
    return {
        ...
        percentChange: 0,
        statsLoading: false,
        resolutionTypeId: resolutionTypeNum,
        genLayerTxHash,
    };
}
