...
export interface UsdlDripStatus {
...
export async function placeBet(
    ...
    const clampedProb = Math.max(1, Math.min(99, Math.round(probability)));
    ...
    await writeContract(wagmiConfig, {
        ...
        args: [betAddress, outcome === 'YES', amountInUnits, clampedProb]
    });
}
...
export async function getUserClaimState(
...
): Promise<UserClaimState> {
    if (isStubbed()) {
        return { claimableAmount: 0, hasClaimed: false };
    }

    const publicClient = getPublicClient();
    const [claimableAmount, hasClaimed] = await Promise.all([
        publicClient.readContract({
            address: betAddress,
            abi: BET_ABI,
    ...
    return {
        claimableAmount: Number(claimableAmount as bigint) / USDL_MULTIPLIER,
        hasClaimed: Boolean(hasClaimed)
    };
}

export function getWalletActionErrorDetails(error: unknown): WalletActionError {
    if (typeof error === 'object' && error !== null) {
        const walletError = error as WalletActionError;
        const nestedCause = getWalletActionErrorDetails(walletError.cause);

        return {
            ...walletError,
            code: walletError.code ?? nestedCause.code,
            message:
                walletError.message ??
                walletError.shortMessage ??
                walletError.details ??
                nestedCause.message,
            shortMessage: walletError.shortMessage ?? nestedCause.shortMessage,
            details: walletError.details ?? nestedCause.details
        };
    }
    return { message: String(error) };
}

export function isUsdlDripLimitError(error: unknown): boolean {
    const { message, shortMessage, details } = getWalletActionErrorDetails(error);
    const haystack = [message, shortMessage, details]
        .filter((value): value is string => Boolean(value))
        .join(' ')
        .toLowerCase();

    return (
        haystack.includes('exceeds 24h mint limit') ||
        haystack.includes('daily drip limit') ||
        haystack.includes('max transaction gas limit') ||
