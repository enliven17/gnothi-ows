...
export async function scheduleResolution(params: ScheduleResolutionRequest): Promise<ScheduleResolutionResponse> {
  ...
  try {
    ...
    const message = typeof error === 'object' && error !== null
      ...
      : String(error);
    console.error(`[Frontend] Schedule failed:`, message);

    // Return a structured error response
    return {
      success: false,
      message: message || 'Failed to communicate with bridge service'
    };
  }
}

/**
 * Get GenLayer oracle tx hash for a market (used by AIConsole)
 */
export async function getGenLayerTxHash(contractAddress: string): Promise<string | null> {
  try {
    const response = await fetch(`${BRIDGE_SERVICE_URL}/oracle/tx/${contractAddress}`);
    if (!response.ok) return null;
    const data = await response.json();
    return data.success ? (data.txHash as string) : null;
  } catch {
    return null;
  }
}
