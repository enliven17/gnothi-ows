...
export class ResolutionQueue {
  ...
  constructor() {
    ...
    // Load existing jobs on startup (fire-and-forget, non-blocking)
    this.loadFromStorage().catch(e => console.warn('[ResolutionQueue] Load error:', e));
  ...
  public addJob(contractAddress: string, endDate: Date, marketTitle: string): string {
    ...
    this.saveToStorage().catch(e => console.warn('[ResolutionQueue] Save error:', e));
  ...
  private scheduleJob(job: ResolutionJob): void {
    ...
    // If the end date is in the past (or very close), execute immediately
    if (endDate <= now) {
      job.status = 'pending';
      console.log(`[Resolution] Past-due job, executing immediately: ${job.marketTitle}`);
      setTimeout(() => this.executeJob(job.id), 2000);
      return;
    }
  ...
  /**
   * Save jobs to persistent storage (Supabase or file fallback)
   */
  private async saveToStorage(): Promise<void> {
    if (isSupabaseConfigured()) {
      // Upsert all in-memory jobs to Supabase
      const rows = Array.from(this.jobs.values()).map(job => ({
        id: job.id,
        contract_address: job.contractAddress,
        end_date: job.endDate.toISOString(),
        market_title: job.marketTitle,
        status: job.status,
        created_at: job.createdAt.toISOString(),
        executed_at: job.executedAt?.toISOString() ?? null,
        error_message: job.errorMessage ?? null,
      }));
      if (rows.length > 0) {
        const { error } = await getSupabaseClient().from('resolution_jobs').upsert(rows);
        if (error) console.warn(`[ResolutionQueue] Supabase upsert error:`, error.message);
      }
      return;
    }
  ...
  /**
   * Load jobs from persistent storage (Supabase or file fallback)
   */
  private async loadFromStorage(): Promise<void> {
    let jobsData: ResolutionJob[] = [];
    ...
    if (isSupabaseConfigured()) {
      ...
      if (error) {
        console.warn(`[ResolutionQueue] Supabase load error:`, error.message);
        return;
      }
      jobsData = (data ?? []).map(row => ({
        id: row.id,
      ...
      try {
        ...
        jobsData = storage.jobs.map(jobData => ({
          ...jobData,
        ...
        console.warn(`[ResolutionQueue] Failed to load from file:`, error);
        return;
      }
    }
    ...
    if (jobsData.length > 0) {
      console.log(`[ResolutionQueue] Loaded ${jobsData.length} jobs`);
    }
  }
