...
async function main() {
  ...
  const probability = process.env.PROBABILITY || "50"; // Confidence in chosen side (1-99)

  if (!tokenAddress || !factoryAddress || !betAddress || !side || !amount) {
    console.log("Usage: MOCK_USDL_ADDRESS=0x... FACTORY_ADDRESS=0x... BET_ADDRESS=0x... SIDE=A AMOUNT=1000000 PROBABILITY=65 npx hardhat run scripts/place-bet.ts --network baseSepolia");
    throw new Error("Missing required env vars: MOCK_USDL_ADDRESS, FACTORY_ADDRESS, BET_ADDRESS, SIDE, AMOUNT");
  }

  const probabilityInt = Number.parseInt(probability, 10);
  if (!Number.isFinite(probabilityInt) || probabilityInt < 1 || probabilityInt > 99) {
    throw new Error(`PROBABILITY must be an integer between 1 and 99. Received: ${probability}`);
  }
  ...
  console.log(`  Probability: ${probabilityInt}%`);
  ...
  const tx = await factory.placeBet(betAddress, onSideA, amount, probabilityInt);
