...
describe("BetCOFI", function () {
  ...
  // Default probability for tests that don't care about SCEM weighting
  const DEFAULT_PROB = 50;
  ...
  describe("Betting via Factory", function () {
    it("Should only allow factory to call betOnSideAViaFactory", async function () {
      await expect(
        bet.connect(bettor1).betOnSideAViaFactory(bettor1.address, USDC_AMOUNT(100), DEFAULT_PROB)
      ).to.be.revertedWith("Only factory can call");
    });

    it("Should only allow factory to call betOnSideBViaFactory", async function () {
      await expect(
        bet.connect(bettor1).betOnSideBViaFactory(bettor1.address, USDC_AMOUNT(100), DEFAULT_PROB)
      ).to.be.revertedWith("Only factory can call");
    });
    ...
    it("Should accumulate bets correctly on side A", async function () {
      ...
      await factory.connect(bettor1).placeBet(await bet.getAddress(), true, bet1, DEFAULT_PROB);
      await factory.connect(bettor1).placeBet(await bet.getAddress(), true, bet2, DEFAULT_PROB);
    ...
    it("Should accumulate bets correctly on side B", async function () {
      ...
      await factory.connect(bettor2).placeBet(await bet.getAddress(), false, betAmount, DEFAULT_PROB);
    ...
    it("Should allow same user to bet on both sides", async function () {
      ...
      await factory.connect(bettor1).placeBet(await bet.getAddress(), true, betA, DEFAULT_PROB);
      await factory.connect(bettor1).placeBet(await bet.getAddress(), false, betB, DEFAULT_PROB);
    ...
    it("Should reject bets after endDate", async function () {
      ...
      await expect(
        factory.connect(bettor1).placeBet(await bet.getAddress(), true, betAmount, DEFAULT_PROB)
    ...
    it("Should emit BetPlacedOnA event", async function () {
      ...
      await expect(factory.connect(bettor1).placeBet(await bet.getAddress(), true, betAmount, DEFAULT_PROB))
        .to.emit(bet, "BetPlacedOnA")
        .withArgs(bettor1.address, betAmount, DEFAULT_PROB);
    });
    ...
    it("Should emit BetPlacedOnB event", async function () {
      ...
      await expect(factory.connect(bettor1).placeBet(await bet.getAddress(), false, betAmount, DEFAULT_PROB))
  ...
  describe("Resolution Request", function () {
    beforeEach(async function () {
      ...
      const betAmount = USDC_AMOUNT(100);
      await usdc.connect(bettor1).approve(await factory.getAddress(), betAmount);
      await factory.connect(bettor1).placeBet(await bet.getAddress(), true, betAmount, DEFAULT_PROB);
    });

    it("Should reject unapproved non-creator resolve()", async function () {
      const endDate = await bet.endDate();
      await time.increaseTo(endDate + 1n);
    ...
    it("Should allow owner to call resolve()", async function () {
      const endDate = await bet.endDate();
      await time.increaseTo(endDate + 1n);

      await expect(
        bet.connect(deployer).resolve()
      ).to.not.be.reverted;
    });

    it("Should allow approved resolver to call resolve()", async function () {
      await factory.setResolverApproval(bettor2.address, true);

      const endDate = await bet.endDate();
      await time.increaseTo(endDate + 1n);

      await expect(
        bet.connect(bettor2).resolve()
      ).to.not.be.reverted;
    });
  ...
  describe("setResolution (from bridge)", function () {
    beforeEach(async function () {
      ...
      await factory.connect(bettor1).placeBet(await bet.getAddress(), true, USDC_AMOUNT(100), DEFAULT_PROB);
      await factory.connect(bettor2).placeBet(await bet.getAddress(), false, USDC_AMOUNT(100), DEFAULT_PROB);
  ...
  describe("Claiming Winnings", function () {
    // Bettor1: 200 on A (prob=50), Bettor2: 100 on B (prob=50), Side A wins
    // SCEM: losersPool=100, S(50,100)=7500 for bettor1
    // bettor1 payout = 200 + 100 = 300
    beforeEach(async function () {
      await usdc.connect(bettor1).approve(await factory.getAddress(), USDC_AMOUNT(200));
      await usdc.connect(bettor2).approve(await factory.getAddress(), USDC_AMOUNT(100));
      await factory.connect(bettor1).placeBet(await bet.getAddress(), true, USDC_AMOUNT(200), DEFAULT_PROB);
      await factory.connect(bettor2).placeBet(await bet.getAddress(), false, USDC_AMOUNT(100), DEFAULT_PROB);
    ...
    it("Should allow winner to claim proportional winnings", async function () {
      ...
      const balanceAfter = await usdc.balanceOf(bettor1.address);
      // Only winner → gets bond (200) + all of losers pool (100) = 300
  ...
  describe("Proportional Winnings Calculation", function () {
    it("Should distribute winnings proportionally among multiple winners", async function () {
      // Bettor1: 200 on A (prob=50), Bettor2: 100 on A (prob=50), Bettor3: 300 on B (prob=50)
      // Both winners have same probability → share proportional to bond
      // losersPool = 300
      // S(50,100) = 7500 for both winners
      // totalWeightedScore = 7500*200 + 7500*100 = 2,250,000
      // bettor1 share = 300 * (7500*200) / 2,250,000 = 300 * 1,500,000 / 2,250,000 = 200
      // bettor2 share = 300 * (7500*100) / 2,250,000 = 100
      await usdc.connect(bettor1).approve(await factory.getAddress(), USDC_AMOUNT(200));
      await usdc.connect(bettor2).approve(await factory.getAddress(), USDC_AMOUNT(100));
      await usdc.connect(bettor3).approve(await factory.getAddress(), USDC_AMOUNT(300));

      await factory.connect(bettor1).placeBet(await bet.getAddress(), true, USDC_AMOUNT(200), DEFAULT_PROB);
      await factory.connect(bettor2).placeBet(await bet.getAddress(), true, USDC_AMOUNT(100), DEFAULT_PROB);
      await factory.connect(bettor3).placeBet(await bet.getAddress(), false, USDC_AMOUNT(300), DEFAULT_PROB);
      ...
      // Equal probability → proportional by bond (same as old behavior)
  ...
  describe("View Functions", function () {
    beforeEach(async function () {
      ...
      await factory.connect(bettor1).placeBet(await bet.getAddress(), true, USDC_AMOUNT(100), DEFAULT_PROB);
      await factory.connect(bettor1).placeBet(await bet.getAddress(), false, USDC_AMOUNT(50), DEFAULT_PROB);
    });
