...
- `BET_FACTORY_ADDRESS` - BetFactoryCOFI address on Base Sepolia
- `GENLAYER_RPC_URL` - GenLayer studionet/mainnet RPC endpoint
- `BRIDGE_SENDER_ADDRESS` - GenLayer BridgeSender contract (deployed on GenLayer)
- `BRIDGE_FORWARDER_ADDRESS` - LayerZero forwarder contract address (optional, for local relay)
- `FORWARDER_NETWORK_RPC_URL` - RPC for the forwarder network (optional, for local relay)
- `SUPABASE_URL` - Supabase project URL (required for prod; omit for in-memory dev mode)
- `SUPABASE_ANON_KEY` - Supabase anon key (required for prod)
